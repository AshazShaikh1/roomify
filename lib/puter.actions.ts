...
export const createProject = async ({ item, visibility = 'private' }: CreateProjectParams): Promise<DesignItem | null | undefined> => {
    ...
    if (!resolvedSource) {
        console.warn('Failed to host source image, skepping save.')
        return null
    }

    const resolvedRender = hostedRender?.url
        ? hostedRender.url
        : (item.renderedImage && isHostedUrl(item.renderedImage) ? item.renderedImage : null);

    const {
        sourcePath: _sosourcePath,
